// Lockstitch.cpp : Defines the functions for the static library.
//
#include "pch.h"
#include "framework.h"
...
#else
	printf("Executable path not supported on this platform.\n");
	return 1;
#endif

	string codeFile = exePath + "Code.txt";
	// Get the current directory path
	/*fs::path current_path = fs::current_path();
	current_path.concat("\\Code.txt");

	ifstream ifs(current_path);*/
...
vector<unsigned char> Lockstitch::divString(vector<unsigned char>& str1, string str2)
{
	...
	int n1 = str1.size() * 4;//one character is a 16 round numbers, 4 bits
	int n2 = str2.length() * 8;//each char is 8 bits
	int n = n1 - n2 + 1;
	if (n1 == 0 || n2 == 0 || n1 < n2)
		return output;
	...
	while (i >= 0)
	{
		...
		if (bLarge)
		{
			...
			//Remove the -1 in front of the dividend
			int j = n1 - 1;
			while (j >= 0)
			{
				if (V1[j] == 1)
...
{
	wstring buffer;
	FILE* f = _wfopen(strFilePath.c_str(), L"rtS, ccs=UTF-8");

	if (f == NULL)
		return L"Error. Failed to open file - " + strFilePath;

	struct _stat fileinfo;
	_wstat(strFilePath.c_str(), &fileinfo);
...
//
//	string extension = filename.substr(indx + 1);
//	string startLocation = encryptData(vec, extension, headSize);
//	while (extension.length() < 8)
//		extension += ' ';
//
//	extension = xorString(prefixData, extension, 8);
//	if (pw.length() < 16)
//		pw.resize(16, ' ');
//
//	pw = xorString(prefixData, pw, 16);
//
//	filename = filename.substr(0, indx) + ".claudo";;
//
...
//	fs.write(extension.c_str(), 8);
//	fs.write((char*)pw.c_str(), 16);
//	fs.close();
//
//	return filename;
}
...
wstring Lockstitch::encryptFile(wstring filename, wstring pw, int headSize)
{
	...
	extension = xorString(prefixData_t, extension, 8);
	if (pw.length() < 16)
		pw.resize(16, ' ');
	...
	fs.write((char*)password.c_str(), 32);
	fs.close();
...
string Lockstitch::decryptFile(string filename, string pw)
{
	...
	//	string password = xorString(prefixData, arr, 16);
	//	if (password != pw)
	//		return ERROR_PW_NOT_MATCH;

	//	//read file extension
	//	copy(content.end() - 8, content.end(), arr);
	//	content.erase(content.end() - 8, content.end());
	//	string extension = xorString(prefixData, arr, 8);
	//	int len = extension.length()-1;
	//	//remove last whitespace
	//	while (extension[len] == ' ')
	//		--len;
	//	extension = extension.substr(0, len + 1);

	//	if (decryptData(content, extension) == 1)
	//		return ERROR_DECRYPT_FAIL;
	...
	//	string outFilePath = filename.substr(0, lastDot) + '.' + extension;//filename.replace(filename.end() -6 - lastDot, filename.end() - lastDot, "Decode");
...
wstring Lockstitch::decryptFile(wstring filename, wstring pw)
{
	try {
		...
		wstring password = xorString(prefixData_t, (wchar_t*)arr, 16);
		if (password != pw) {
		...
		copy(content.end() - 16, content.end(), arr);
		content.erase(content.end() - 16, content.end());
		wstring extension = xorString(prefixData_t, (wchar_t*)arr, 8);
		int len = extension.length() - 1;
		//remove last whitespace
		while (extension[len] == ' ')
			--len;
